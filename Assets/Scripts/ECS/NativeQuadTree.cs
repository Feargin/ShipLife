...
namespace NativeQuadTree
{
	...
	public struct QuadElement<T> : IComponentData
	{
		public float2 pos;
		public T Entity;
	}
	...
	public unsafe partial struct NativeQuadTree<T> : IDisposable where T : unmanaged
	{
		...
		) : this()
		{
			...
			lookup = Create(UnsafeUtility.SizeOf<int>(),
				UnsafeUtility.AlignOf<int>(),
				totalSize,
				allocator,
				NativeArrayOptions.ClearMemory);

			nodes = Create(UnsafeUtility.SizeOf<QuadNode>(),
				UnsafeUtility.AlignOf<QuadNode>(),
				totalSize,
				allocator,
				NativeArrayOptions.ClearMemory);

			elements = Create(UnsafeUtility.SizeOf<QuadElement<T>>(),
